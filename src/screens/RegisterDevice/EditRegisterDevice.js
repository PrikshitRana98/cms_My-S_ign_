...
const EditRegisterDevice = ({ navigation,route }) => {
  ...
  useEffect(()=>{
    ...
    setState(prev => {
      return {...prev,mpIdentity: clientGeneratedDeviceIdentifier,
        mpName: deviceName,
        mpOsType: deviceOs,
  ...
  const btnValidateForm = () => {
    ...
    // deviceManagerService.addDevice(postData, succussCallBack, failureCallBack);
  ...
  const btnAddPanelDetails = async()=>{
    ...
    const succussCallBack = async (response) => {
      ...
      setIsLoading(false);
      if (response?.result?.length > 0) {
        Alert.alert("Info!", 'Device edit successfully', [
          {
            text: "Ok",
            onPress: () => {
    ...
    const failureCallBack = (error) => {
      setIsLoading(false);
      console.log("error add device", error?.response?.response);
      if(error?.response?.data?.result?.length>0){
        alert(error?.response?.data?.result[0]?.message);
      }else if(error?.response?.data?.message){
        alert(error?.response?.data?.message);
...
export default EditRegisterDevice;
