...
const MediaActions = ({
  ...
  return (
    <View style={[Styles.iconBackContainer, containerStyle]}>
      ...
      {authorization.includes(PREVILAGES.MEDIA.VIEW_MEDIA)&&(mediaType==="IMAGE"||mediaType==="VIDEO"||mediaType==="AUDIO"||mediaType==="PDF"||mediaType==="PPT") &&<TouchableOpacity onPress={()=>handleViewMedia()} style={[Styles.iconBackView, iconContainerStyle]}>
        <Ionicons name="eye" size={20} color={themeColor.themeColor} />
      </TouchableOpacity>}

      {header!="Archived"&&(mediaType==="IMAGE"||mediaType==="VIDEO"||mediaType==="AUDIO"||mediaType==="PDF"||mediaType==="PPT")&&authorization.includes(PREVILAGES.MEDIA.EDIT_MEDIA) &&<TouchableOpacity 
