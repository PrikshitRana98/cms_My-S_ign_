...
const CampainDtringDetails = ({ navigation, route }) => {
  ...
  const renderCampaignList = ({ item, index }) => {
    let campaignTags1 = "--";
    if (
      campaignItem?.campaignTags != "--" &&
      campaignItem?.campaignTags.length > 0
    ) {
      campaignTags1 = campaignItem?.campaignTags
        .map(function (val) {
    ...
    return (
      <View style={Styles.renderContainer}>
        <View style={[Styles.nameView, { width: "25%" }]}>
          <AppText style={Styles.nameText}>{"image"}</AppText>
        </View>
        <View style={[Styles.nameView, { width: "25%" }]}>
          <View
            style={{
              height: 29,
              width: 70,
              backgroundColor: campaignItem?.backgroundColor || "#000",
            }}
          />
        </View>
        <View style={[Styles.nameView, { width: "25%" }]}>
          <AppText style={Styles.nameText}>
            {campaignItem?.transparencyInPercentage?.toString()}
          </AppText>
        </View>
        <View style={[Styles.nameView, { width: "25%" }]}>
          ...
          return (
            <View style={Styles.renderContainer1}>
              <View style={[Styles.nameView, { width: "20%" }]}>
                ...
                />
                <AppText style={[Styles.nameText]}>{mdata?.name}</AppText>
              </View>
              <View style={[Styles.nameView, { width: "20%" }]}>
  ...
  const RegionrenderCampaignList = ({ item, index }) => {
    const totalDuration = item?.mediaArra?.reduce(
      (acc, a) => acc + parseInt(a.defaultDurationInSeconds),
      0
    );
    return (
      <>
        ...
        {selectedCampaignDrop === item.campaignRegionName ? (
          <RCampaignChildList data={item.mediaArra} />
        ) : null}
      </>
    );
  };
...
export default CampainDtringDetails;
