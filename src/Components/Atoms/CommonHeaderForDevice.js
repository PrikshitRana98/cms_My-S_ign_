...
import BulkActionButton from "./BulkActionButtonForMP";
import { useSelector } from "react-redux";
import { PREVILAGES } from "../../Constants/privilages";

const CommonHeaderTitleAction = ({ title = "",isBulkOptionShow, titleTextStyle = {},btnOpenModelType=()=>{},currPage=0,totalItemCount=0}) => {
  ...
  return (
    <View style={Styles.headerView}>
      ...
      {
        isBulkOptionShow &&
      <BulkActionButton renderDelete={authorization.includes(PREVILAGES.DEVICE.DELETE_DEVICE)} title={title} pageName={title} btnOpenModelType={deleteData}/>
      }
    </View>
  );
};
