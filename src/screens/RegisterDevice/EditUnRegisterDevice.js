...
const EditUnRegisterDevice = ({ navigation,route }) => {
  ...
  const btnValidateForm = () => {
    ...
    setIsLoading(true);
    const succussCallBack = async (response) => {
      console.log("success add device--377", response);
      setIsLoading(false);
      if (response?.code === 200) {
        ...
        if (response?.result?.length > 0) {
          if (response?.result[0].field == "locationId") {
    ...
    deviceManagerService.editUnRegDevice(params, succussCallBack, failureCallBack);
  };
  // End step:1================================================
  // step2================================================
...
export default EditUnRegisterDevice;
