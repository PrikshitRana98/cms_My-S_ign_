...
import { CampaignManagerService } from "./CompainApi";
  ...
  export default function CmpPreviwe({ navigation, route }) {
    ...
    const appStateChangeHandler = async(newState) => {
      console.log("app event upload222222",newState)
      if (newState === 'active') {  
    ...
    const [totalDurationOfCampaignInSeconds,setTotalDurationOfCampaignInSeconds] = useState(0);
            ...
            playerDataArr.map(async (pData) => {
              await btnGetMediaById(pData.mediaDetailId)
                .then((res) => {
                  ...
                  if (res.status === "OK") {
                    mediaArr1.push({...res.data.mediaDetails[0],displayMode:pData.displayMode});
                  } else {
    ...
    const getCamapaignDetails = async (selectedCampaignId) => {
      console.log("selectedCampaignIdselectedCampaignId",selectedCampaignId)
      setIsLoading(true);
      let slugId = await getStorageForKey("slugId");
      const params = {
    ...
    const formateCamapign = async (res) => {
      if (res && res?.data) {
        ...
        // console.log("apiResp.backgroundImageContentId",apiResp.backgroundImageContentId)
          ...
          await btnGetMediaById(apiResp.backgroundImageContentId).then((res) => {
            if (res?.status === "OK") {
              if (
                ...
                console.log("bg image Data==cover>",JSON.stringify(res.data.mediaDetails[0].mediaDimension))
              }
            }
          });
  
          console.log("background tranperency===>",{
            url: mediaURL,
        ...
        if (apiResp.audios && apiResp.audios.length > 0) {
          ...
          const playSongWithInterval= async(arr, index)=> {
            ...
            if (index < arr.length) {
              ...
              await btnGetMediaById(arr[index].contentId).then((res) => {
                ...
                if (res?.status === "OK") {
                  if (
                    ...
                    startAudio(res.data.mediaDetails[0].audioUrl)
                    setSongChangeTime(res.data.mediaDetails[0].defaultDurationInSeconds)
                  }
                }
              });
              audiosArr = {
              ...
              index++;
              setTimeout(function () {
                // console.log("btnGetMediaById(a.contentId)============>",index);
    ...
    const RegionrenderCampaignList = ({ item, index }) => {
      console.log("logg loction canedit",campaignData[0].canEditLocationBasedPlaylist)
      ...
      return (
        <>
          <View style={Styles.renderContainer1}>
            ...
            <View style={[Styles.nameView, { width: "20%" }]}>
              <AppText style={Styles.nameText}>{campaignData[0].canEditLocationBasedPlaylist?"Yes":"No"}</AppText>
            </View>
            <View style={[Styles.nameView, { width: "20%" }]}>
              <AppText style={Styles.nameText}>
          ...
          <View
              ...
              >
                ...
                imageStyle={{
                  ...
                  opacity: (backgroundImage.transparencyInPercentage?backgroundImage.transparencyInPercentage:1),
                }}
                source={
                  backgroundImage?.url != "" ? { uri: backgroundImage.url } : null
                }
                ...
                {selectedCampaign != null &&
                  selectedCampaignRegions?.map((item, index) => {
                    // console.log("item.regionTransparencyInPercentage=center==>",item.isAudioEnabled)

                    const viewOpacity=item.regionTransparencyInPercentage<1?item.regionTransparencyInPercentage:1
                    const opa=viewOpacity<0.3?0.4:viewOpacity;
                    return (
                      <TouchableOpacity
                        activeOpacity={viewOpacity}
                        key={"campaign" + index}
                        style={[Styles.regionContainer(item),{opacity:viewOpacity}]}
                        
                      >
                        
                        <CmpDetailMedia sliderValue={parseInt(sliderValue)} mediaArr={item.mediaArra} isMute={item.isAudioEnabled} />
                      </TouchableOpacity>
                    );
                  })}
                </ImageBackground>
